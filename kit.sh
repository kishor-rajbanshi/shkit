...
_ansi() {
    ...
    for _attr in "$@"; do
        ...
        *)
        ...
        _seq="${_seq:+$_seq;}$_code"
    done

    [ "$_INTERACTIVE" ] && printf '\033[%sm' "$_seq"
}
...
_die() {
    ...
    while getopts ':m:c:' _opt; do
        ...
        c) _code="$OPTARG" ;;
        :)
            _error "_die: -$OPTARG: expected argument"
            exit 2
            ;;
        ?)
    ...
    if ! _integer "$_code" || _negative "$_code"; then
        _error "_die: -c: expected non-negative integer, got: '$_code'"
        exit 2
    fi
...
_trap() {
    ...
    if [ -z "$1" ]; then
        _error "_trap: invalid argument: COMMAND: expected non-empty string, got: '$1'"
        return 2
    fi

    if _str_contains "$1" '§'; then
    ...
    for _sig in "$@"; do
        ...
        if [ -z "${_existing:-}" ]; then
            eval "_trap_registry_${_sig}=\"\${1}\""
        else
            eval "_trap_registry_${_sig}=\"\${_existing}§\${1}\""
        fi
...
_line() {
    ...
    while getopts ':c:w:' _opt; do
        ...
        w) _width="$OPTARG" ;;
        :)
            _error "_line: -$OPTARG: expected argument"
            return 2
            ;;
        ?)
    ...
    if [ "${#_char}" -ne 1 ]; then
        _error "_line: -c: expected 1 character, got: '$_char'"
        return 2
    fi

    if ! _integer "$_width" || _negative "$_width"; then
        _error "_line: -w: expected non-negative integer, got: '$_width'"
        return 2
    fi
...
_confirm() {
    ...
    while getopts ':p:d:' _opt; do
        ...
        d) _default="$OPTARG" ;;
        :)
            _error "_confirm: -$OPTARG: expected argument"
            return 2
            ;;
        ?)
    ...
    if [ -z "$_default" ]; then
        ...
        *)
            _error "_confirm: -d: expected y | Y | yes | Yes | YES | n | N | no | No | NO, got: '$_default'"
            return 2
            ;;
        esac
    fi
...
_input() {
    ...
    while getopts ':p:h:d:e:' _opt; do
        ...
        e) _error="$OPTARG" ;;
        :)
            _error "_input: -$OPTARG: expected argument"
            return 2
            ;;
        ?)
...
_uuid() {
    if [ "$#" -ne 0 ]; then
        _error "_uuid: expected 0 arguments, got: $#"
        return 2
    fi
    ...
    if _exists uuidgen; then
        ...
        printf '%s-%s-%s' "$(date +%s)" "$$" \
            "$(od -An -N4 -tx4 /dev/urandom 2>/dev/null | tr -d ' \n')"
    fi
}
...
_now() {
    if [ "$#" -ne 0 ]; then
        _error "_now: expected 0 arguments, got: $#"
        return 2
    fi
...
# Compute elapsed seconds since a given start time.
# Usage: _elapsed [-h] <start_seconds>
# Options:
#   -h  Show time in human-readable format (e.g. 2m 34s)
_elapsed() {
    _human=""
    ...
    while getopts ':h' _opt; do
        ...
        h) _human=1 ;;
        ?)
            _error "_elapsed: invalid option: '-$OPTARG'"
            return 2
            ;;
        esac
    done
    ...
    if [ "$#" -ne 1 ]; then
        _error "_elapsed: expected 1 argument (start_seconds), got: $#"
        return 2
    fi
